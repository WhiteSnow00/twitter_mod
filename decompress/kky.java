...
{
    public static final f3a c;
    public final Context a;
    public final fyy b;
    
    static {
        c = new f3a("AssetPackStorage");
        final TimeUnit days = TimeUnit.DAYS;
        days.toMillis(14L);
        days.toMillis(28L);
    }
    
    public kky(final Context a, final fyy b) {
        this.a = a;
        this.b = b;
    }
    
    public static long d(final File file, final boolean b) {
        if (!file.exists()) {
            return -1L;
        }
        final ArrayList list = new ArrayList();
        if (b && file.listFiles().length > 1) {
            kky.c.m("Multiple pack versions found, using highest version code.", new Object[0]);
        }
        try {
            for (final File file2 : file.listFiles()) {
                if (!file2.getName().equals("stale.tmp")) {
                    list.add(Long.valueOf(file2.getName()));
                }
            }
        }
        catch (final NumberFormatException ex) {
            kky.c.k(ex, "Corrupt asset pack directories.", new Object[0]);
        }
        if (list.isEmpty()) {
            return -1L;
        }
        ...
        if (file.listFiles() != null) {
            if (file.listFiles().length > 1) {
                ...
                for (File[] listFiles = file.listFiles(); i < listFiles.length; ++i) {
                    file = listFiles[i];
                    if (!file.getName().equals(String.valueOf(d)) && !file.getName().equals("stale.tmp")) {
                        j(file);
                    }
                }
            }
    ...
    public final List<File> h() {
        ...
        try {
            ...
            kky.c.j("Could not process directory while scanning installed packs. %s", ex);
        }
        return list;
    }
    
    public final int k(final String s, int int1, final long n) throws IOException {
        final File file = new File(this.p(s, int1, n), "merge.tmp");
        if (!file.exists()) {
            return 0;
        }
        final Properties properties = new Properties();
        ...
        try {
            ...
            if (properties.getProperty("numberOfMerges") != null) {
                try {
                    ...
                    throw new xpy("Merge checkpoint file corrupt.", (Exception)ex);
                }
            }
            throw new xpy("Merge checkpoint file corrupt.");
        }
        finally {
            try {
                fileInputStream.close();
    ...
    public final String u(final String s) throws IOException {
        ...
        if (!file.exists()) {
            kky.c.i("Pack not found with pack name: %s", s);
            return null;
        }
        final File file2 = new File(file, String.valueOf(this.b.a()));
        if (!file2.exists()) {
            kky.c.i("Pack not found with pack name: %s app version: %s", s, this.b.a());
            return null;
        }
        ...
        if (listFiles != null) {
            ...
            if (length != 0) {
                if (length > 1) {
                    kky.c.j("Multiple pack versions found for pack name: %s app version: %s", s, this.b.a());
                    return null;
                }
                return listFiles[0].getCanonicalPath();
            }
        }
        kky.c.i("No pack version found for pack name: %s app version: %s", s, this.b.a());
        return null;
    }
