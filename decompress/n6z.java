import java.io.IOException;
import java.util.HashMap;

// 
// Decompiled by Procyon v0.6.0
// 

public final class n6z implements j4j<ygz>
{
    public static final n6z a;
    
    static {
        a = new n6z();
        final vpy vpy = new vpy(0);
        vpy.b = 1;
        final qqy f = vpy.f();
        final HashMap hashMap = new HashMap();
        final vpy l = sbl.l(f, hashMap, f, hashMap, 0);
        l.b = 2;
        final qqy f2 = l.f();
        final HashMap hashMap2 = new HashMap();
        final vpy i = sbl.l(f2, hashMap2, f2, hashMap2, 0);
        i.b = 3;
        final qqy f3 = i.f();
        final HashMap hashMap3 = new HashMap();
        final vpy j = sbl.l(f3, hashMap3, f3, hashMap3, 0);
        j.b = 4;
        final qqy f4 = j.f();
        final HashMap hashMap4 = new HashMap();
        final vpy k = sbl.l(f4, hashMap4, f4, hashMap4, 0);
        k.b = 5;
        final qqy f5 = k.f();
        final HashMap hashMap5 = new HashMap();
        final vpy m = sbl.l(f5, hashMap5, f5, hashMap5, 0);
        m.b = 6;
        final qqy f6 = m.f();
        final HashMap hashMap6 = new HashMap();
        final vpy l2 = sbl.l(f6, hashMap6, f6, hashMap6, 0);
        l2.b = 7;
        final qqy f7 = l2.f();
        final HashMap hashMap7 = new HashMap();
        final vpy l3 = sbl.l(f7, hashMap7, f7, hashMap7, 0);
        l3.b = 8;
        final qqy f8 = l3.f();
        final HashMap hashMap8 = new HashMap();
        final vpy l4 = sbl.l(f8, hashMap8, f8, hashMap8, 0);
        l4.b = 9;
        final qqy f9 = l4.f();
        final HashMap hashMap9 = new HashMap();
        eta.i(f9, hashMap9, f9, hashMap9);
    }
}
